...
async function testStakingProgram(){
    ...
    let delegatedAuthPda = await PublicKey.findProgramAddress([Buffer.from("authority")], programId)
    let stakeStatePda = await PublicKey.findProgramAddress([wallet.publicKey.toBuffer(), nft.tokenAddress.toBuffer()], programId)
    ...
    // redeeming rewards
    let mintAuth = await PublicKey.findProgramAddress([Buffer.from("mint")], programId)
    ...
    const redeemTxid = await program.methods.redeem()
    .accounts({
        user: wallet.publicKey,
        nftTokenAccount: nft.tokenAddress,
        stakeState: stakeStatePda[0],
    ...
    const unstakeTxid = await program.methods.unstake()
    .accounts({
        user: wallet.publicKey,
        nftTokenAccount: nft.tokenAddress,
        nftMint: nft.mintAddress,
