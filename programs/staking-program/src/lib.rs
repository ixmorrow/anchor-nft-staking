...
use anchor_spl::token::{Approve, Revoke, Token, MintTo, TokenAccount, Mint};
...
declare_id!("6s3muoRZnpcQ5jjUqPz1Ra9kyWAqiDbXexMmJJegAjRV");
...
pub mod staking_program {
    ...
    pub fn stake(ctx: Context<Stake>) -> Result<()> {
        ...
        msg!("NFT token account: {:?}", ctx.accounts.stake_state.token_account);
        msg!("User pubkey: {:?}", ctx.accounts.stake_state.user_pubkey);
        msg!("Stake state: {:?}", ctx.accounts.stake_state.stake_state);
        msg!("Stake start time: {:?}", ctx.accounts.stake_state.stake_start_time);
        msg!("Time since last redeem: {:?}", ctx.accounts.stake_state.last_stake_redeem);
    ...
    pub fn redeem(ctx: Context<Redeem>) -> Result<()> {
        ...
        let cpi_approve_ctx = CpiContext::new_with_signer(cpi_approve_program, cpi_accounts, signer);
        msg!("CPI to mint ix on token program");
        token::mint_to(cpi_approve_ctx, redeem_amount.try_into().unwrap())?;
        ctx.accounts.stake_state.last_stake_redeem = clock.unix_timestamp;
        msg!("Updated last stake time: {:?}", ctx.accounts.stake_state.last_stake_redeem);
